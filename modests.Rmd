...
output$map <- renderLeaflet({
  ...
  pal <- colorNumeric(
    ...
    na.color = 'grey',
    domain = range(toplo[[colsel]], na.rm = T)
  )
  
  out <- mapview(toplo, homebutton = F, label = round(toplo[[colsel]], 2), color = pal(toplo[[colsel]])) %>% 
    .@map %>% 
    removeMouseCoordinates() %>% 
    addLegend("topright", pal = pal, values = toplo[[colsel]],
              title = colsel,
              opacity = 1
    )
...
column(12, 
       column(3, 
              selectInput('mosel', 'Select month:', choices = sort(unique(flowmetprf$mo)))
       ),
       
       column(3, 
              selectInput('colsel', 'Select value to show:', choices = list(`predicted` = 'est', `CV` = 'cv'))
       )
)
```
