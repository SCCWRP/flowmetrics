...
# input
dtsl <- as.Date(c('2090-07-01', '2095-07-01', '2100-07-01'))
toproc <- crossing(
  dtsl = dtsl,
  COMID = comid_pnts$COMID
)
...
# predict future flowmetrics for three dates, canesm2 ----------------------------
...
# predict future flowmetrics for three dates, ccsm4 ----------------------------
...
# predict future flowmetrics for three dates, miroc5 ----------------------------
...
# combine all future flowmetrics across three models and three dates ------
...
flowmetdt2 <- list(
    canesm2 = canesm2flowmetdt2,
    ccsm4 = ccsm4flowmetdt2,
    miroc5 = miroc5flowmetdt2
  ) %>% 
  enframe('model', 'data') %>% 
  unnest('data') %>% 
  group_by(var, COMID, dtsl, set) %>% 
  dplyr::select(-model, -mo) %>% 
  summarise_all(mean, na.rm = T)

save(flowmetdt2, file = 'data/flowmetdt2.RData', compress = 'xz')
